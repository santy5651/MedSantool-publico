...
import { Trash2, Upload, Download, FileText, Image as ImageIcon, MessageSquareText, Lightbulb, Info, AlertCircle, CheckCircle, Settings2, FileEdit, Star, Brain, ClipboardPlus, ListChecks } from 'lucide-react';
...
export function useHistoryStore() {
  ...
  const addHistoryEntry = useCallback(async (entryData: Omit<HistoryEntry, 'id' | 'timestamp'>) => {
    try {
      ...
      toast({ title: "Guardado en Historial", description: `Entrada para ${entryData.module.replace(/([A-Z])/g, ' $1').trim()} guardada.` });
    } catch (error) {
  ...
  const exportHistory = useCallback(async () => {
    ...
    try {
      const dataToExport = historyEntries.map(({ id, ...rest }) => rest); 
  ...
  const importHistory = useCallback(async (file: File, mode: 'replace' | 'add') => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await db.history.bulkAdd(processedEntries as HistoryEntry[]);
