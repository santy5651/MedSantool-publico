...
/**
 * @fileOverview Analyzes medical images (radiographs and EKGs) to identify potential anomalies, generate a summary of findings, and provide a detailed reading in Spanish.
 *
...
const AnalyzeMedicalImageInputSchema = z.object({
  photoDataUri: z
    .string()
    .describe(
      "A medical image (radiograph, EKG) as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
});
...
const AnalyzeMedicalImageOutputSchema = z.object({
  ...
  radiologistReading: z.string().describe('A detailed reading of the image in Spanish. For a radiograph, this is a formal report. For an EKG, this is a cardiologist\'s interpretation (rhythm, rate, axis, intervals, morphology, conclusion).'),
});
...
- Si la imagen es una radiografía, actuarás como un **radiólogo experto**.
...
2.  Proporciona una **lectura detallada** de la imagen en español.
    -   Para una radiografía, la lectura debe ser similar a un informe radiológico formal.
