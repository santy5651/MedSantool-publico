...
const GeneratePhysicalExamOutputSchema = z.object({
    physicalExamText: z.string().describe('Un texto corto que describe únicamente los hallazgos patológicos probables en un examen físico, basado en el texto clínico proporcionado. Cada hallazgo debe estar en una nueva línea y seguir el formato "SISTEMA: HALLAZGO".'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `Eres un médico experto en semiología. Tu tarea es analizar el siguiente texto de la enfermedad actual de un paciente y generar una lista de **hallazgos patológicos probables** para un examen físico dirigido.
...
-   Basa tus sugerencias estrictamente en la información del texto clínico.
