...
const GeneratePhysicalExamInputSchema = z.object({
  ...
  additionalAnalysis: z
    ...
    .optional()
    .describe(
      'Un análisis clínico enfocado o resumen del caso, ingresado manualmente o autocompletado. Usarlo como contexto adicional para los hallazgos patológicos.'
    ),
});

const GeneratePhysicalExamInputWithKeySchema =
  GeneratePhysicalExamInputSchema.extend({
    apiKey: z.string().optional().describe('User provided Google AI API key.'),
  });

export type GeneratePhysicalExamInput = z.infer<
  typeof GeneratePhysicalExamInputSchema
>;
export type GeneratePhysicalExamInputWithKey = z.infer<
  typeof GeneratePhysicalExamInputWithKeySchema
>;
...
const GeneratePhysicalExamOutputSchema = z.object({
  physicalExamText: z
    .string()
    .describe(
      'Un texto completo del examen físico, basado en una plantilla de hallazgos normales pero modificado para incluir hallazgos patológicos relevantes según los diagnósticos proporcionados.'
    ),
});
export type GeneratePhysicalExamOutput = z.infer<
  typeof GeneratePhysicalExamOutputSchema
>;

export async function generatePhysicalExam(
  input: GeneratePhysicalExamInputWithKey
): Promise<GeneratePhysicalExamOutput> {
  const ai = getGenkitInstance(input.apiKey);

  const prompt = ai.definePrompt({
    name: `generatePhysicalExamPrompt_${Date.now()}`,
