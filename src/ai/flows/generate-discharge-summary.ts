...
const GenerateDischargeSummaryInputSchema = z.object({
  formulaMedica: z
    ...
    .optional()
    .describe(
      'Medications for discharge. Example: "ACETAMINOFEN, TABLETAS DE 500 MG, TOMAR 2 TABLETAS, CADA 8 HORAS, POR 10 DIAS". AI should correct typos and format.'
    ),
  conciliacionMedicamentosa: z
    .string()
    .optional()
    .describe(
      'Medication reconciliation. Example: "LOSARTAN, TABLETAS 50MG, 1 TABLETA VO CADA DIA". AI should correct typos and format.'
    ),
  laboratoriosControl: z
    .string()
    .optional()
    .describe(
      'Outpatient follow-up labs. If empty, default to "NO SE ENVIA LABORATORIOS DE CONTROL".'
    ),
  proximoControl: z
    .string()
    .optional()
    .describe(
      'Next medical follow-up. Example: "VALORACION Y SEGUIMIENTO POR CONSULTA EXTERNA CON: MEDICINA GENERAL, EN 10 DIAS". AI should improve phrasing if short input like "ORTOPEDIA 30 DIAS" is given.'
    ),
  tramites: z
    .string()
    .optional()
    .describe(
      'Relevant administrative procedures, typically EPS and type. Example: "NUEVA EPS SUBSIDIADO".'
    ),
  incapacidad: z
    .string()
    .optional()
    .describe(
      'Medical leave/disability days, if applicable. This field should be ignored if "tramites" indicates a subsidized EPS.'
    ),
  signosAlarma: z
    .string()
    .optional()
    .describe(
      'Alarm signs for the patient. Can be auto-filled or manually entered.'
    ),
  indicacionesDieta: z
    .string()
    .optional()
    .describe('Dietary indications. Can be auto-filled or manually entered.'),
  cuidadosGenerales: z
    .string()
    .optional()
    .describe('General care instructions. Can be auto-filled or manually entered.'),
  recomendacionesGenerales: z
    .string()
    .optional()
    .describe(
      'General recommendations. Can be auto-filled or manually entered.'
    ),
  condicionesSalida: z
    .string()
    .optional()
    .describe(
      'General discharge conditions. AI should correct typos and improve wording.'
    ),
});

const GenerateDischargeSummaryInputWithKeySchema =
  GenerateDischargeSummaryInputSchema.extend({
...
>;
export type GenerateDischargeSummaryInputWithKey = z.infer<
  typeof GenerateDischargeSummaryInputWithKeySchema
>;

const GenerateDischargeSummaryOutputSchema = z.object({
  generatedSummary: z
    .string()
    .describe('The fully formatted discharge summary text.'),
});
...
export async function generateDischargeSummary(
  input: GenerateDischargeSummaryInputWithKey
): Promise<GenerateDischargeSummaryOutput> {
  const ai = getGenkitInstance(input.apiKey);

  const prompt = ai.definePrompt({
    name: `generateDischargeSummaryPrompt_${Date.now()}`,
...
2.  **Fórmula Médica:** Si la plantilla muestra medicamentos en esta sección, revisa y corrige errores ortográficos. Reorganiza cada medicamento para que siga el formato exacto: "NOMBRE DEL MEDICAMENTO, PRESENTACIÓN Y CONCENTRACIÓN (ej. TABLETAS DE 500 MG), INSTRUCCIÓN DE DOSIFICACIÓN (ej. TOMAR 2 TABLETAS), VÍA, FRECUENCIA, DURACIÓN (si aplica)". Es crucial que todos estos componentes estén presentes. Por ejemplo, si la entrada es "Losartan 50mg 1 tab vo cada dia", la salida debería ser "LOSARTAN, TABLETAS DE 50 MG, TOMAR 1 TABLETA, VÍA ORAL, CADA DÍA". Otro ejemplo: si la entrada es "acetaminofen tab 500mg 2 tab c/8h x 10d", la salida debería ser "ACETAMINOFEN, TABLETAS DE 500 MG, TOMAR 2 TABLETAS, VÍA ORAL, CADA 8 HORAS, POR 10 DÍAS". Otro ejemplo: si la entrada es "acetaminofen 1 gramo cada 8 horas", la salida debe ser "ACETAMINOFEN, TABLETAS DE 500 MG, TOMAR 2 TABLETAS, VÍA ORAL, CADA 8 HORAS". La IA debe inferir la presentación y cantidad de tabletas más comunes si solo se da la dosis total. Lista cada medicamento en una nueva línea. Si la plantilla indica "NO SE FORMULA MEDICAMENTOS AL EGRESO", mantenlo así.
3.  **Conciliación Medicamentosa:** Similar a la Fórmula Médica. Si la plantilla muestra medicamentos, revisa y corrige errores ortográficos. Reorganiza cada medicamento para que siga el formato exacto: "NOMBRE DEL MEDICAMENTO, PRESENTACIÓN Y CONCENTRACIÓN (ej. TABLETAS DE 100 MG), INSTRUCCIÓN DE DOSIFICACIÓN (ej. TOMAR 1 TABLETA), VÍA, FRECUENCIA, DURACIÓN (si aplica)". Por ejemplo, si la entrada es "ASA 100mg tomar 1 tab vo cada dia", la salida debería ser "ASA (ÁCIDO ACETILSALICÍLICO), TABLETAS DE 100 MG, TOMAR 1 TABLETA, VÍA ORAL, CADA DÍA". La IA debe inferir la presentación y cantidad de tabletas más comunes si solo se da la dosis total. Lista cada medicamento en una nueva línea. Si la plantilla indica "NO TIENE MEDICAMENTOS DE CONCILIACIÓN", mantenlo así.
...
`,
  ...
  // Pre-process input to handle empty strings and derived logic
  const processedInput: GenerateDischargeSummaryInput = {};
  for (const key in input) {
    if (key === 'apiKey') continue; // Don't process the api key
  ...
  // Logic for disability based on subsidized EPS
  if (
    processedInput.tramites &&
    processedInput.tramites.toLowerCase().includes('subsidiado')
  ) {
    processedInput.incapacidad = undefined; // Don't show disability if subsidized
  }

  // Ensure fields that should be uppercase are, if they have content
  const fieldsToUppercase: Array<keyof GenerateDischargeSummaryInput> = [
    'signosAlarma',
    'indicacionesDieta',
    'cuidadosGenerales',
    'recomendacionesGenerales',
  ...
  fieldsToUppercase.forEach(field => {
    if (processedInput[field] && typeof processedInput[field] === 'string') {
      const defaultValueForSigns =
        '⚠️ ***SIGNOS DE ALARMA***\nCONSULTE CON SU MÉDICO ANTE CUALQUIER SÍNTOMA NUEVO O EMPEORAMIENTO DE SU CONDICIÓN ACTUAL.';
      const defaultValueForDiet =
        '🍽️ ***INDICACIONES SOBRE LA DIETA***\nNO SE PUEDEN DAR INDICACIONES DIETÉTICAS ESPECÍFICAS SIN INFORMACIÓN DIAGNÓSTICA. CONSULTE A SU MÉDICO O NUTRICIONISTA.';
      const defaultValueForCare =
        '⚕️ ***CUIDADOS GENERALES***\nES IMPORTANTE SEGUIR LAS INDICACIONES GENERALES DE SU MÉDICO Y MANTENER UN ESTILO DE VIDA SALUDABLE. PARA CUIDADOS ESPECÍFICOS, CONSULTE A SU MÉDICO.';
      const defaultValueForRecs =
        '***RECOMENDACIONES GENERALES***\nNO SE HA PROPORCIONADO INFORMACIÓN DIAGNÓSTICA ESPECÍFICA (DIAGNÓSTICOS VALIDADOS O TEXTO MANUAL). ES FUNDAMENTAL CONSULTAR CON SU MÉDICO PARA RECIBIR INDICACIONES PERSONALIZADAS.';

      let isDefaultText = false;
      if (
        field === 'signosAlarma' &&
        processedInput[field]?.toUpperCase() === defaultValueForSigns.toUpperCase()
      )
        isDefaultText = true;
      if (
        field === 'indicacionesDieta' &&
        processedInput[field]?.toUpperCase() === defaultValueForDiet.toUpperCase()
      )
        isDefaultText = true;
      if (
        field === 'cuidadosGenerales' &&
        processedInput[field]?.toUpperCase() === defaultValueForCare.toUpperCase()
      )
        isDefaultText = true;
      if (
        field === 'recomendacionesGenerales' &&
        processedInput[field]?.toUpperCase() === defaultValueForRecs.toUpperCase()
      )
        isDefaultText = true;

      if (!isDefaultText) {
        processedInput[field] = (
          processedInput[field] as string
        ).toUpperCase();
      }
    }
  });
