...
const GenerateMedicalOrderInputSchema = z.object({
  orderType: z
    .enum(['OBSERVACIÓN MENOR A 6 HORAS', 'OBSERVACIÓN', 'HOSPITALIZACIÓN', 'EGRESO'])
    .describe('Tipo de orden médica.'),
  oxygen: z
    .string()
    .default('NO REQUIERE OXÍGENO')
    .describe(
      'Requerimientos de oxígeno. Ejemplo: "Oxígeno por cánula nasal a 2 L/min" o "NO REQUIERE OXÍGENO".'
    ),
  isolation: z
    .string()
    .default('NO REQUIERE AISLAMIENTO')
    .describe(
      'Tipo de aislamiento. Ejemplo: "Aislamiento de contacto" o "NO REQUIERE AISLAMIENTO".'
    ),
  diet: z
    .string()
    .optional()
    .describe(
      'Dieta del paciente. Aplicable solo si orderType es HOSPITALIZACIÓN. Ejemplo: "Dieta absoluta", "Dieta líquida".'
    ),
  medicationsInput: z
    .string()
    .describe(
      "Lista de medicamentos, uno por línea. Formato esperado: Nombre del medicamento, presentación y concentración, dosis, vía, frecuencia, duración opcional. Ejemplo: Acetaminofén, tabletas de 500 mg, administrar 1 gramo, vía oral, cada 8 horas."
    ),
  medicationReconciliationInput: z
    .string()
    .describe(
      "Detalles de conciliación medicamentosa. Ejemplo: Losartán, tabletas de 50 mg, administrar 50 mg, via oral, cada día. Si no tiene, ingresar 'NO TIENE CONCILIACIÓN MEDICAMENTOSA'."
    ),
  specialtyFollowUp: z
    .string()
    .optional()
    .describe(
      'Especialidad médica que realizará el seguimiento durante la hospitalización. Ejemplo: "Cardiología", "Medicina Interna".'
    ),
  fallRisk: z
    .string()
    .default(
      'RIESGO DE CAIDAS Y LESIONES POR PRESION SEGUN ESCALAS POR PERSONAL DE ENFERMERIA'
    )
    .describe(
      'Indicación sobre riesgo de caídas y lesiones por presión.'
    ),
  paduaScale: z
    .string()
    .describe("Puntaje en la escala de Padua. Ejemplo: '3 puntos', 'NO APLICA'."),
  surveillanceNursing: z
    .object({
      ...
      cuidadosDrenajesQuirurgicos: z
        .boolean()
        .describe('Cuidados de drenajes quirúrgicos.'),
      cuidadosTraqueostomia: z
        .boolean()
        .describe('Cuidados de traqueostomía.'),
      controlGlicemicoTurno: z
        .boolean()
        .describe('Control glicémico por turno.'),
      controlGlicemicoAyunas: z
        .boolean()
        .describe('Control glicémico en ayunas.'),
      thermalCurve: z.boolean().describe('Curva térmica.'),
      hojaNeurologica: z.boolean().describe('Hoja neurológica.'),
      realizarCuraciones: z
        .boolean()
        .describe('Realizar curaciones y cuidados de heridas.'),
      restriccionHidrica800: z
        .boolean()
        .describe('Restricción hídrica a 800 cc/24 horas.'),
      controlLiquidosAdminElim: z
        .boolean()
        .describe('Control de líquidos administrados y eliminados.'),
      registroBalanceHidrico24h: z
        .boolean()
        .describe('Registro de balance hídrico de 24 horas.'),
  ...
  transferConditions: z
    .enum([
      'Grupo 1: Camillero o auxiliar',
      'Grupo 2: Médico general',
      'Grupo 3: Médico general por paciente intubado',
      'NO APLICA',
    ])
    .describe('Condiciones de traslado del paciente.'),
  specialConsiderations: z
    .string()
    .optional()
    .describe('Notas o consideraciones especiales adicionales.'),
});

const GenerateMedicalOrderInputWithKeySchema = GenerateMedicalOrderInputSchema.extend(
  {
    apiKey: z.string().optional().describe('User provided Google AI API key.'),
  }
);

export type GenerateMedicalOrderInput = z.infer<
  typeof GenerateMedicalOrderInputSchema
>;
export type GenerateMedicalOrderInputWithKey = z.infer<
  typeof GenerateMedicalOrderInputWithKeySchema
>;
...
const GenerateMedicalOrderOutputSchema = z.object({
  generatedOrderText: z
    .string()
    .describe(
      'El texto completo de las órdenes médicas generales, formateado profesionalmente en español, con correcciones de redacción aplicadas y EN MAYÚSCULAS, incluyendo los asteriscos en los títulos principales.'
    ),
});
export type GenerateMedicalOrderOutput = z.infer<
  typeof GenerateMedicalOrderOutputSchema
>;

export async function generateMedicalOrder(
  input: GenerateMedicalOrderInputWithKey
): Promise<GenerateMedicalOrderOutput> {
  const ai = getGenkitInstance(input.apiKey);

  const prompt = ai.definePrompt({
    name: `generateMedicalOrderPrompt_${Date.now()}`,
