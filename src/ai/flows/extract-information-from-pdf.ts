...
export type ExtractInformationFromPdfInput = z.infer<
  typeof ExtractInformationFromPdfInputSchema
>;
export type ExtractInformationFromPdfInputWithKey = z.infer<
  typeof ExtractInformationFromPdfInputWithKeySchema
>;
...
export type ExtractInformationFromPdfOutput = z.infer<
  typeof ExtractInformationFromPdfOutputSchema
>;

export async function extractInformationFromPdf(
  input: ExtractInformationFromPdfInputWithKey
): Promise<ExtractInformationFromPdfOutput> {
  const ai = getGenkitInstance(input.apiKey);

  const prompt = ai.definePrompt({
    name: `extractInformationFromPdfPrompt_${Date.now()}`,
